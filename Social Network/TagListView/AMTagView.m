...
- (void)layoutSubviews
{
    ...
    NSLog(@"%@",self.labelText.text);
    if (self.labelText.text.length >= 38) {
        [self.labelText setFrame:CGRectMake(self.labelText.frame.origin.x,self.labelText.frame.origin.y, 230, self.labelText.frame.size.height)];
        [self.labelText setNumberOfLines:2];
    }
