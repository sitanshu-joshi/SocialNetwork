...
{
    __weak CustomSlideViewController *_c;
}
...
- (id)initWithFrame:(CGRect)frame controller:(CustomSlideViewController*)controller
{
...
@interface CustomSlideViewController()<UIGestureRecognizerDelegate>
{
...
@synthesize isFromFavorites,isFromHome,isFromInfoCard;
...
- (void)prepareForSegue:(CustomSlideViewControllerSegue *)segue sender:(id)sender
{
    // $ using a custom segue we can get access to the storyboard-loaded rear/front view controllers
    // the trick is to define segues of type CustomSlideViewControllerSegue on the storyboard
    // connecting the CustomSlideViewController to the desired front/rear controllers,
    ...
    // was used to instantiate the CustomSlideViewController
    
    // $ none of this would be necessary if Apple exposed "relationship" segues for container view controllers.

    NSString *identifier = segue.identifier;
    if ( [segue isKindOfClass:[CustomSlideViewControllerSegue class]] && sender == nil )
    {
        if ( [identifier isEqualToString:SWSegueRearIdentifier] )
        {
            segue.performBlock = ^(CustomSlideViewControllerSegue* rvc_segue, UIViewController* svc, UIViewController* dvc)
            {
...
- (void)_dispatchSetFrontViewPosition:(FrontViewPosition)frontViewPosition animated:(BOOL)animated
{
    ...
    __weak CustomSlideViewController *theSelf = self;
...
-(void)_dispatchSetRightViewController {
...
- (void)_setFrontViewPosition:(FrontViewPosition)newPosition withDuration:(NSTimeInterval)duration
{
    ...
    @catch (NSException *exception) {
        NSLog(@"CustomSlideViewController->_setFrontViewPosition %@", exception);
    }
...
- (void)_setFrontViewController:(UIViewController*)newFrontViewController
{
    ...
    NSLog(@"Animation Qeue = %@",_animationQueue);
...
- (void (^)(void))_rearViewDeploymentForNewFrontViewPosition:(FrontViewPosition)newPosition
{
    ...
    _rearViewPosition = newPosition;
        ...
        MenuOptionsViewController *menuOption = (MenuOptionsViewController *)_rearViewController;
...
@implementation UIViewController(CustomSlideViewController)

- (CustomSlideViewController*)revealViewController
{
    UIViewController *parent = self;
    Class revealClass = [CustomSlideViewController class];
...
#pragma mark - CustomSlideViewControllerSegue Class

@implementation CustomSlideViewControllerSegue
